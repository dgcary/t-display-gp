...
const char* HomeAssistantConfigValidationResult::message() const {
  switch (error) {
    ...
    case HomeAssistantConfigError::BASE_URL: return "Home Assistant URL must be canonical HTTPS without trailing slash";
    case HomeAssistantConfigError::TOKEN: return "Home Assistant token is missing or too long";
    case HomeAssistantConfigError::CA_CERT: return "Home Assistant CA certificate is missing or invalid";
...
HomeAssistantConfigValidationResult validateHomeAssistantConfig(const HomeAssistantConfig& config) {
  ...
  if (!config.baseUrl.empty()) {
    if (config.baseUrl.size() > MAX_BASE_URL_BYTES || !startsWith(config.baseUrl, "https://") ||
        ...
        config.baseUrl.find('#', 8) != std::string::npos) {
  ...
  if (config.caCert.find("-----BEGIN CERTIFICATE-----") == std::string::npos ||
      config.caCert.find("-----END CERTIFICATE-----") == std::string::npos) {
