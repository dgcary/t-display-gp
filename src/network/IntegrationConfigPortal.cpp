...
struct IntegrationConfigPortal::Impl {
  ...
  void loginBambu() {
    ...
    if (input.email.empty() || effectivePassword.empty()) {
      server.send(400, "application/json; charset=utf-8", "{\"message\":\"请输入账号和密码；中国区可使用手机号，Global 使用邮箱；密码留空只会保留已经保存过的密码\"}");
      return;
    }
  ...
  void saveBambu() {
    ...
    if (!validateBambuConfig(updated).ok()) {
      server.send(400, "application/json; charset=utf-8", "{\"message\":\"Bambu 配置无效：启用时需要有效账号以及密码或有效 Token\"}");
      return;
    }
