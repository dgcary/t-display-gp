...
bool readJwtUid(JsonObjectConst root, std::string& uid) {
  ...
  for (const char* field : fields) {
    ...
    if (value.is<const char*>()) return readBoundedString(value, BambuConfigLimits::CLOUD_USER_ID, uid);
    if (value.is<long long>()) {
      const long long number = value.as<long long>();
      if (number < 0) return false;
      uid = std::to_string(number);
      return true;
    }
...
bool parseBambuDeviceList(const std::string& body, std::vector<BambuCloudDevice>& out) {
  ...
  if (root["data"].is<JsonArrayConst>()) array = root["data"].as<JsonArrayConst>();
  else if (root["devices"].is<JsonArrayConst>()) array = root["devices"].as<JsonArrayConst>();
  else return false;
  ...
  for (JsonVariantConst item : array) {
    ...
    if (!safeSerial(device.serial) || containsSerial(candidate, device.serial)) return false;
